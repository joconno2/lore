...
# crash_recovery on ALL arms (incl. base): AA's own crashes (RecursionError/
# AssertionError) on diverged paths confound the comparison (e.g. a 109k game
# crashed to 16k). This isolates the survival oracle's effect on a crash-robust
# base; it's a structural robustness fix, not the LLM decision being tested.
lore_patches.apply_crash_recovery()
if mode == "mock":
    lore_patches.apply_survival_oracle(mock=True)
elif mode == "llm":
    lore_patches.apply_survival_oracle(mock=False)
...
json.dump({"seed": seed, "mode": mode, "score": s.get("score"), "turns": s.get("turns"),
           ...
           "surv_query": C.get("surv_query"),
           "surv_actions": {k: C[k] for k in C if k.startswith("surv_") and k != "surv_query"},
           "end_reason": str(s.get("end_reason"))[:70],
