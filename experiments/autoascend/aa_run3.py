"""AutoAscend + sokoban patch + oracle descent gate. Args: seeds(csv) out mock(1/0)."""
import sys, json, time
import gym
import lore_patches

seeds = [int(s) for s in sys.argv[1].split(",")] if len(sys.argv) > 1 else [113, 123, 128, 132, 140, 147]
OUT = sys.argv[2] if len(sys.argv) > 2 else "/workspace/aa_descent.json"
MOCK = (sys.argv[3] != "0") if len(sys.argv) > 3 else True

print("sokoban:", lore_patches.apply(), flush=True)
print("descent gate:", lore_patches.apply_descent_gate(mock=MOCK), flush=True)
...
for seed in seeds:
    ...
    s = env.get_summary()
    stats = {k: v for k, v in s.items() if "lore_descent" in str(k)}
    r = {"seed": seed, "score": s.get("score"), "level_num": s.get("level_num"),
         "xl": s.get("experience_level"), "turns": s.get("turns"),
         "end_reason": str(s.get("end_reason"))[:80], "descent_stats": stats,
