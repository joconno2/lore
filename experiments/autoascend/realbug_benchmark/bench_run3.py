"""Runner v3 (on trx): full autonomous_debug5 pipeline per real bug, localization
scored as 'any changed function in top-6 retrieved' (match-any, no bundled confound)."""
...
def debug(symptom,src):
    ...
    STOP={"turns","games","level","never","the","its","then","dies","gets","other","kill","while",
        "bot","and","reaches","single","set","with","work","well","some","case","being","blocked",
        "measure","longer","fix","fixes","fixed","bug","add","small","make","improve","better"}
    ...
    gate=llm("A NetHack bot (a strong competition winner) failure:\n"+symptom+
        "\n\nRelevant retrieved functions:\n"+blob+"\n\nIs this caused by a SPECIFIC FIXABLE BUG in one "
        "of these functions, or is it FUNDAMENTAL game difficulty (no specific bug)? Answer with ONLY the "
    ...
    return {"search_terms":g,"retrieved":[k for k,_ in top],"gate":gate.strip()[:220],"diagnosis":diag.strip()[:600]}
res=[]
for i,c in enumerate(CASES):
    print(f"[{i+1}/{len(CASES)}] {c['sha'][:7]} {c['symptom'][:48]}",file=sys.stderr)
    try:
        out=debug(c["symptom"],checkout(c["sha"]))
    except Exception as e: out={"error":str(e),"retrieved":[]}
    hit=bool(set(c["changed_funcs"]) & set(out.get("retrieved",[])))
    out.update({"sha":c["sha"],"symptom":c["symptom"],"changed_funcs":c["changed_funcs"],
                "groundable":c["groundable"],"loc_hit_any":hit})
    res.append(out); print("   gate:",out.get("gate","")[:60],"| loc_any:",hit,file=sys.stderr)
json.dump(res,open("/tmp/aa_bench3_results.json","w"),indent=1)
